...
/**
 * A generic command-line interface for 6.035 compilers. This class provides
...
public class CLI {
    ...
    /**
     * DEFAULT: produce default output. SCAN: scan the input and stop. PARSE:
     * scan and parse input, and stop. INTER: produce a high-level intermediate
     * representation from the input, and stop. This is not one of the segment
     * targets for Fall 2006, but you may wish to use it for your own purposes.
     * ASSEMBLY: produce assembly from the input.
     */
    public enum Action {
        DEFAULT, ABOUT, SCAN, PARSE, INTER, ASSEMBLY
    };
     ...
     * the input and output files to null, the target to DEFAULT, and the extra
     * array to a new empty Vector.
     */
    static {
        outfile = null;
        infile = null;
     ...
     * accordingly. <BR>
     *
     * <TT>-t / --target= <I>target</I></TT> sets the CLI.target field based on
     * the <I>target</I> specified. <BR>
     * <TT>scan</TT> or <TT>scanner</TT> specifies Action.SCAN <TT>parse</TT>
     * specifies Action.PARSE <TT>inter</TT> specifies Action.INTER
    ...
    public static void parse(String args[], String optnames[]) {
                    ...
                    if (optsList[j].equals("all")) {
                        ...
                        for (int k = 0; k < optnames.length; k++) {
                            if (optsList[j].equals(optnames[k])) {
                                opts[j] = true;
                            } else if (optsList[j].charAt(0) == '-' || optsList[j].substring(1).equals(optnames[k])) {
                                opts[j] = false;
                            }
                        }
                    }
        ...
        if (!targetStr.equals("")) {
            ...
            if (targetStr.equals("scan"))
                target = Action.SCAN;
            else if (targetStr.equals("parse"))
                target = Action.PARSE;
            else if (targetStr.equals("inter"))
                target = Action.INTER;
            else if (targetStr.equals("assembly"))
